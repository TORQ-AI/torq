...
import { STRAVA_TOKEN_URL } from '../fetch-activity/constants';
 ...
 * @returns {Promise<StravaTokenResponse>} Promise resolving to parsed token response
 * @throws {Error} Throws AuthError with 'MALFORMED_RESPONSE' code if JSON parsing fails
 * @internal
 */
const parseTokenResponse = async (response: Response): Promise<StravaTokenResponse> => {
  try {
...
const exchangeToken = async (
  ...
  const response = await fetchTokenResponse(STRAVA_TOKEN_URL, body.toString());
