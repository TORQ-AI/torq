 ...
 *
 * @param {ActivitySignals} signals - Activity signals to validate
 * @returns {ActivitySignalsValidationResult} Validation result with errors and optional sanitized signals
 *
 * @remarks
 * Validates:
 ...
 */
const validateActivitySignals = (signals: ActivitySignals): ActivitySignalsValidationResult => {
  const errors: string[] = [];
  ...
  // Validate intensity
  const validIntensities: Array<ActivitySignals['intensity']> = ['low', 'medium', 'high'];
  if (!validIntensities.includes(signals.intensity)) {
    errors.push(`Intensity must be one of: ${validIntensities.join(', ')}`);
  }
  
  // Validate elevation
  const validElevations: Array<ActivitySignals['elevation']> = ['flat', 'rolling', 'mountainous'];
  if (!validElevations.includes(signals.elevation)) {
    errors.push(`Elevation must be one of: ${validElevations.join(', ')}`);
  }
  
  // Validate time of day
  const validTimeOfDay: Array<ActivitySignals['timeOfDay']> = ['morning', 'day', 'evening', 'night'];
  ...
  if (signals.weather !== undefined) {
    const validWeather: Array<NonNullable<ActivitySignals['weather']>> = ['sunny', 'rainy', 'cloudy', 'foggy'];
  ...
  const sanitized: ActivitySignals | undefined = valid
    ? undefined
    : {
        ...signals,
